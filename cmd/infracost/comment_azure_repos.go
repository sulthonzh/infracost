	...
	cmd := &cobra.Command{
      ...
      infracost comment azure-repos --repo-url https://dev.azure.com/my-org/my-project/_git/my-repo --pull-request 3 --path infracost.json --azure-access-token $AZURE_ACCESS_TOKEN`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !dryRun {
				onFailure, _ := cmd.Flags().GetBool("comment-on-failure")

				if onFailure && policyFailure == nil {
					cmd.Println("Comment not posted to Azure Repos, policy check passed (--comment-on-failure was specified)")
					return nil
				}
